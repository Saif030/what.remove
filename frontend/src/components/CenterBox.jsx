...
const CenterBox = () => {
    const { removeBg } = useContext(DataContext);
    ...
    return (
        <div className="w-full sm:h-[75vh] h-screen flex flex-col justify-center items-center sm:flex-row sm:p-6 p-4">
            <div className="w-full sm:w-1/2 h-full flex flex-col justify-center">
                <h1 className="sm:text-6xl text-4xl font-semibold leading-tight">Remove the<br /><span className="font-bold bg-gradient-to-r from-[#f7c548] to-[#ffe08a] bg-clip-text text-transparent">background</span> from<br />images for free.</h1>
                ...
                <input onChange={(e) => (removeBg(e.target.files[0]))} type="file" name="image" id="file-input" className="hidden" />
